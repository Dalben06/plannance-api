...
import { CalendarEventFilters, CalendarWeekStartsOn } from "../../domain/calendarEvent.js";
import { addDays, parseMonthRangeUtc, startOfWeek } from "../../utils/date.js";
...
export const createCalendarDayService = (
  ...
  const listOfMonthDays = (date: string, startsOn: CalendarWeekStartsOn): CalendarDay[] => {
    const range = parseMonthRangeUtc(date);
    if (!range?.start) return [];

    const gridStart = startOfWeek(range?.start, startsOn);

    return Array.from({ length: 42 }, (_, i) => addDays(gridStart, i)).map((d) => ({
      date: d,
      events: [],
      expense: 0,
      income: 0,
